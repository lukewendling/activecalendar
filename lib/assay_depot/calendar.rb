...
#        html << %(        button         :    "#{options[:id]}_trigger",  // trigger for the calendar, button ID\n)
      ...
      def depot_datetime_select(options = {})
        ...
        html << %(<input type="text" name="#{options[:name]}" value="#{datetime}" class="#{options[:class]} text-input" id="#{options[:id]}" size="#{options[:size]}" />\n)
    ...
    module DateHelper
#      Rails 2.1 fix: added html_options arg, but not currently used
